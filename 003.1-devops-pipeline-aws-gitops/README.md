...
```
# sudo init 6, sistemi güvenli şekilde yeniden başlatmak için kullanılan klasik bir Linux komutudur.
# init 6 komutunda init, sistemi yönetip çalışma seviyelerini değiştiren süreçtir; 6 ise sistemi yeniden başlatmayı temsil eden runlevel numarasıdır.
sudo init 6 
```
ya da
```
...
AWS EC2 makinemizi dış dünyaya açtık.
...
İsim olarak aşağıdakini yazdık.
...
Bu komut hem /etc/hostname dosyasını günceller hem de sistemi yeniden başlatmadan hostname’i aktif hâle getirir.
*/
```
...
docker login -u huseyin11 -p dckr_passwrd
...
Jenkinsfile içine bu komutları ekledik.
        ...
        stage("Trivy Scan") {
            steps {
                script {
                    if (isUnix()) {
                        sh ('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image huseyin11/003.1-devops-pipeline-aws:latest --no-progress --scanners vuln  --exit-code 0 --severity HIGH,CRITICAL --format table')
                     } else {
                        bat ('docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image huseyin11/003.1-devops-pipeline-aws:latest --no-progress --scanners vuln  --exit-code 0 --severity HIGH,CRITICAL --format table')
                    }
                }
            }
        }
...
Elimizle terminalden imajları tek tek silmek komutları
					...
					docker rmi huseyin11/003-devops-pipeline-aws:1.0.10
					docker rmi huseyin11/003-devops-pipeline-aws:1.0.11
					docker rmi huseyin11/003-devops-pipeline-aws:1.0.12
					docker rmi huseyin11/003-devops-pipeline-aws:1.0.13
					docker rmi huseyin11/003-devops-pipeline-aws:1.0.14
					docker rmi huseyin11/003-devops-pipeline-aws:latest
...
```
docker login -u huseyin11 -p dckr_passwd
```
